...
# Function to sanitize CDR3 nucleotide sequences
sanitize_cdr3_nt <- function(df, col = "CDR3") {
  if (!col %in% colnames(df)) return(df)
  df[[col]] <- toupper(df[[col]])
  ok <- grepl("^[ACGTN]+$", df[[col]])
  if (!all(ok)) {
    warning(sprintf("Dropping %d rows with non-ACGTN in %s (e.g., placeholders).",
                    sum(!ok), col))
    df <- df[ok, , drop = FALSE]
  }
  df
...
if (sample_level_run == TRUE || intracohort_run == TRUE || db_run == TRUE) {
  ...
  for (current_sample in files) {
    ...
    if (input_format == "TRUST4") {
      ...
      sample_table <- sanitize_cdr3_nt(sample_table, "CDR3")          
    ...
    if (dim(sample_table)[1] != 0) {
      ...
      for (current_chain in chains_search) {
        ...
        if (current_chain == "IGH" && input_format == "ADAPTIVE" && cdr3nt_skip == FALSE) {
          chain_table_unprocessed <- sanitize_cdr3_nt(chain_table_unprocessed, "CDR3")
